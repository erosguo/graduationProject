        ...
        <section class="content">
            ...
            <div class="box box-primary">
                ...
                <c:if test="${empty pageList.dataList}">
                    <h1 class="text-center">您还没有任何消息</h1>
                </c:if>
                <c:if test="${not empty pageList.dataList}">
                    <div class="box-body">
                        ...
                        <div class="table-box">
                            ...
                            <table id="dataList" class="table table-bordered table-striped table-hover dataTable">
                                <thead>
                                <tr>
                                    <th class="sorting_desc">消息类型[降序]</th>
                                    <td>消息来源</td>
                                    <td>消息内容</td>
                                    <td>可选操作</td>
                                </tr>
                                ...
                                <tbody>
                                    <c:forEach items="${pageList.dataList}" var ="newMessage">
                                    <tr>
                                        <td>
                                            <c:if test="${newMessage.messageType==1}">
                                                好友消息
                                            </c:if>
                                            <c:if test="${newMessage.messageType==2}">
                                                社团消息
                                            </c:if>
                                            <c:if test="${newMessage.messageType==3}">
                                                系统消息
                                            </c:if>
                                        </td>
                                        <td>${newMessage.messageFrom}</td>
                                        <td>${newMessage.messageContent}</td>
                                        <td>
                                            <button class="btn btn-success" onclick="location.href='${pageContext.request.contextPath}/User/deleteMessage?messageId=${newMessage.messageId}'">删除</button>
                                        </td>
                                    </tr>
                    ...
                    <div class="box-footer">
                        ...
                        <div class="box-tools pull-right">
                            <ul class="pagination">
                                <li>
                                    <a aria-label="Previous" href="${pageContext.request.contextPath}/User/showMessageList?page=1&size=${pageList.pageSize}">首页</a>
                                </li>
                                <c:if test="${pageList.currentPage>1}">
                                    <li>
                                        <a href="${pageContext.request.contextPath}/User/showMessageList?page=${pageList.currentPage-1}&size=${pageList.pageSize}">上一页</a>
                                    </li>
                                </c:if>

                                <c:forEach begin="1" end="${pageList.totalPage}" var="pageNum">
                                    <li>
                                        <a href="${pageContext.request.contextPath}/User/showMessageList?page=${pageNum}&size=${pageList.pageSize}">${pageNum}</a>
                                    </li>
                                </c:forEach>
                                <c:if test="${pageList.currentPage<pageList.totalPage}">
                                    <li>
                                        <a href="${pageContext.request.contextPath}/User/showMessageList?page=${pageList.currentPage+1}&size=${pageList.pageSize}">下一页</a>
                                    </li>
                                </c:if>

                                <li>
                                    <a aria-label="Previous" href="${pageContext.request.contextPath}/User/showMessageList?page=${pageList.totalPage}&size=${pageList.pageSize}">尾页</a>
                                </li>
                            </ul>
                        </div>
...
<script>
    function changePageSize() {
        ...
        //向服务器发送请求，改变每页显示条数
        location.href="${pageContext.request.contextPath}/User/showMessageList?page=1&size="+pageSize;
    }
</script>
</html>
